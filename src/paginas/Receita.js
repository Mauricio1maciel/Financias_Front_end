...
//       });
//   }, [dados, filtroRecebido, filtroCategoria, ano, mes]);
  
//   const dadosOrdenados = useMemo(() => {
//     let dadosSensiveisAOrdenacao = [...dadosFiltrados];
...
//   }, [dadosFiltrados, configOrdenacao]);
...
//     if (configOrdenacao.chave !== chaveColuna) {
//       return null;
//     }
//     return configOrdenacao.direcao === 'ascendente' ? 
...
//     worksheet['!cols'] = [{ wch: 18 }, { wch: 30 }, { wch: 15 }, { wch: 30 }, { wch: 15 }, { wch: 20 }];
...
//   const totalReceitas = dadosFiltrados.reduce((acc, curr) => acc + Number(curr.value), 0);
//   const aReceberVencidas = dadosFiltrados.filter((d) => !d.paid && d.due_date_vencimento && dayjs(d.due_date_vencimento).tz(dayjs.tz.guess()).isBefore(dayjs().startOf('day')));
...
//           <label htmlFor="filtroStatusSelect" className="form-label">Status:</label>
//           <select id="filtroStatusSelect" className="form-select" value={filtroRecebido} onChange={(e) => setFiltroRecebido(e.target.value)}>
...
//             <button onClick={handleExportExcel} className="btn btn-success w-100" disabled={dadosOrdenados.length === 0}>
...
//           <strong>{dadosOrdenados.length}</strong> no total<br />
//           <span className="text-muted">{formatarValor(totalReceitas)}</span>
//         </div>
//         <div className="me-3 mb-2 text-warning">
...
//             <tr>
//               <th onClick={() => solicitarOrdenacao('paid')} style={{ cursor: 'pointer' }}>
//                 Recebido <IconeOrdenacao chaveColuna="paid" />
//               </th>
//               <th onClick={() => solicitarOrdenacao('group_name')} style={{ cursor: 'pointer' }}>
//                 Nome da Receita <IconeOrdenacao chaveColuna="group_name" />
//               </th>
//               <th onClick={() => solicitarOrdenacao('value')} style={{ cursor: 'pointer' }}>
//                 Valor <IconeOrdenacao chaveColuna="value" />
//               </th>
//               <th onClick={() => solicitarOrdenacao('due_date_vencimento')} style={{ cursor: 'pointer' }}>
//                 Previsão/Recebimento <IconeOrdenacao chaveColuna="due_date_vencimento" />
//               </th>
//               <th></th>
//             </tr>
//           </thead>
...
export default function Receitas() {
  ...
  return (
    <div className="container mt-4"> 
      ...
      <div className="bg-light p-3 rounded d-flex flex-wrap justify-content-between mb-4 border">
        <div className="me-3 mb-2">
          <strong>{dadosOrdenados.length}</strong> {filtroRecebido !== 'todos' ? 'visíveis' : 'no total'}<br />
