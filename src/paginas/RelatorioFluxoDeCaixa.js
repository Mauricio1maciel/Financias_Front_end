...
export default function RelatorioFluxoDeCaixa({ movimentacoesDoPeriodo, mes, ano }) {
  ...
  // Ajustado de curr.value para curr.valor
            ...
            <tbody>
              {receitas.map(r => (
                <tr key={`rec-${r.id}`}>
                  <td>{formatarData(r.data_pagamento || r.due_date_vencimento)}</td>
                  <td>{r.group_name}{r.description ? ` - ${r.description}` : ''}</td>
                  {/* Ajustado de r.value para r.valor */}
                  <td className="text-end text-success">{formatarValor(r.valor)}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <tbody>
              {despesas.map(d => (
                <tr key={`desp-${d.id}`}>
                  <td>{formatarData(d.data_pagamento || d.due_date_vencimento)}</td>
                  <td>{d.group_name}{d.description ? ` - ${d.description}` : ''}</td>
                  {/* Ajustado de d.value para d.valor */}
                  <td className="text-end text-danger">{formatarValor(d.valor)}</td>
                </tr>
              ))}
            </tbody>
          </table>
